...
\section*{Dot Product}
	...
	\vspace{-1cm}
	\begin{Enum}
		\item Find two vectors orthogonal to $\vec a=\mat{1\\-3}$.  Could you find two linearly independent vectors 
			that are orthogonal to $\vec a$?
		\item Find two vectors orthogonal to $\vec b=\mat{1\\-3\\4}$.  Could you find two linearly independent vectors 
			that are orthogonal to $\vec b$?
...
\section*{Projections}
	...
	%%%
	\begin{Enum}
		\item In this picture $\|\vec a\|=4$ and $\theta = \pi/6$.  
			Find $\|\Proj_{\vec b}\vec a\|$ and $\|\Perp_{\vec b}\vec a\|$.
		\item If $\vec b=\begin{bmatrix}6\\0\end{bmatrix}$, write down 
				$\Proj_{\vec b}\vec a$ and $\Perp_{\vec b}\vec a$ in column vector
				form.
		\item If $\vec b=\begin{bmatrix}-4\\0\end{bmatrix}$, write down 
				$\Proj_{\vec b}\vec a$ and $\Perp_{\vec b}\vec a$ in column vector
				form.
		\item If $\vec b=\begin{bmatrix}3\\1\end{bmatrix}$, write down 
				$\Proj_{\vec b}\vec a$ and $\Perp_{\vec b}\vec a$ in column vector
...
\section*{Lines, Planes, Normals, Equations}
	...
	\begin{Def}
		A \emph{normal vector} to a line (or plane or hyperplane) is one that is orthogonal to it.
	\end{Def}
	...
	\begin{Enum}
		...
		\item Prove that any plane of the form $\vec n\cdot \vec x=0$ is a subspace.
	\end{Enum}
...
\subsection*{Arbitrary Lines and Planes}
	...
	\begin{Enum}
		...
		\item Verify the vector $\overline {p_1p_2}$ is perpendicular to $\vec n$.
...
\end{Enum}
	...
	that distributes with respect to vector addition and scalar multiplication.  That is $T$
