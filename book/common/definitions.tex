...
\begin{SaveDefinition}[key=CompositionofFunctions, title={Composition of Functions}]
	Let $f:A\to B$ and $g:B\to C$. The \emph{composition} of $g$ and $f$, notated $g\circ f$\index[symbols]{$\circ$},
...
\begin{SaveDefinition}[
	...
	In this case, we call $g$ an \emph{inverse}\index[symbols]{$f^{-1}$} of $f$ and write
	\[
		f^{-1}=g.
	\]
\end{SaveDefinition}
