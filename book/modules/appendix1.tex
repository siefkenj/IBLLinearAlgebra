...
%	were five up until now.}.
	...
	\begin{example}
		...
		The solution is $\mat{a\\b\\c}=\mat{5\\-4\\4}$. Since this is the only solution to the system, the solution set is $\Set*{\mat{5\\-4\\4}}$.
	\end{example}
	...
	\begin{example}
		...
		The solution vector is $\mat{t\\s\\r}=\mat{11\\-9\\-2}$ and so the solution set is $\Set*{\mat{11\\-9\\-2}}$.
	\end{example}
	...
	\begin{example}
		...
		This time, let us use only row reduction and the equivalent system
		\[
			\systeme{
				a+3b+2c=1,
				b+c=0,
		...
		\end{align*}
		The solution is $\mat{a\\b\\c}=\mat{5\\-4\\4}$ and the solution set is $\Set*{\mat{5\\-4\\4}}$.
		
		Notice how we found the same solution and solution set using only row reduction.
	\end{example}
	...
	\begin{example}
		...
		There exists no vector $\mat{x\\y\\z}$ that solves the system and as such, the solution set is $\Set*{}$.
	\end{example}
