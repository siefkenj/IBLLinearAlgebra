	...
	\begin{example}
		...
		Paying attention to the coordinates and treating $x$, $y$, and $z$ as coefficients\index{Linear combination!coefficient of}, we can see that $\systeme{2x+3y+z=2,0x+y-z=-1}$ corresponds to the vector equation
		\[
...
%	were five up until now.}.
	...
	the \emph{coefficients} of the variables $t$, $s$, and $r$ and the \emph{constants} in
	the corresponding system of equations. An (optional) vertical line
	...
	\begin{example}
		...
		\end{align*}
		The third row reveals that $c=4$ and we substitute that into the second row to find $b=-4$. Now we can substitute $b=-4$ and $c=4$ into the first row and we obtain $a=5$.
		
		As a vector, the solution is $\mat{5\\-4\\4}$ and since it is unique, the solution set is $\Set*{\mat{5\\-4\\4}}$.
	\end{example}
	...
	\begin{example}
		...
		\]
		Again, we row reduce the corresponding augmented matrix to find an equivalent system\index{System of linear equations!equivalent} that is easier to find the solution set.
		\begin{align*}
			{\footnotesize
			\systeme[tsr]{
				3t+s+13r=-2,
		...
		\end{align*}
		Our equivalent system reveals $r=-2$ which we can substitute back into the first and second rows to find that $t=11$ and $s=-9$, respectively.
		
		The solution vector is $\mat{11\\-9\\-2}$ and so the solution set is $\Set*{\mat{11\\-9\\-2}}$.
	\end{example}
	...
	\begin{example}
		We return to the earlier system
		\[
			\systeme[abc]{
				a+3b+2c=1,
				2a+7b+5c=2,
		...
		\]
		except now let's try row reducing as much as possible.
		\begin{align*}
			{\footnotesize
			\systeme[abc]{
				a+3b+2c=1,
		...
		\end{align*}
		We obtain the same solution vector as before: $\mat{5\\-4\\4}$.
	\end{example}
	...
	\begin{example}
		...
		\]
		As usual we rewrite the system as an augmented matrix then row reduce.
		\begin{align*}
			{\footnotesize
			\systeme[xyz]{
				x+z=4,
		...
		\end{align*}
		The equation $0x + 0y + 0z = 14$ can never be true and so the system is inconsistent. There exists no vector $\mat{x\\y\\z}$ that solves the system and as such, the solution set is $\Set*{}$.
	\end{example}
