...
\SavedDefinitionRender{ImageofaSet}\index[definitions]{Set!image}
...
\SavedDefinitionRender{LinearTransformation}\index[definitions]{Linear transformation}
...
\begin{theorem}
	If $T:\R^n\to\R^m$ is a linear transformation, then $T$ takes lines\index{Line!under a linear transformation} to lines (or points).
\end{theorem}
\begin{proof}
...
\begin{theorem}
	If $T:\R^n\to\R^m$ is a linear transformation, then $T$ takes subspaces\index{Subspace!under a linear transformation} to subspaces.
\end{theorem}
\begin{proof}
...
Every linear transformation from $\R^n$ to $\R^m$ has a matrix\index{Linear transformation!matrix representation}, and we can use basic algebra to find
