...
\begin{example}
	Compute the determinant of $M=\mat{1 & 6 \\ 2 & 7}$ using both the $2\times 2$ formula and
	the decomposition into elementary matrices.
	...
	\[
		\mat{1 & -6 \\ 0 & 1}\mat{1 & 0 \\ 0 & -\frac{1}{5}}\mat{1 & 0 \\ -2 & 1}M=\mat{1&0\\0&1},
	\]
	and so
	\[
		M=\mat{1 & 0 \\ 2 & 1}\mat{1 & 0 \\ 0 & -5}\mat{1 & 6 \\ 0 & 1}=E_1E_2E_3.
	\]

	$E_1$ and $E_3$ both have determinant $1$, and $E_2$ has determinant $-5$. Thus,
	\[
		\det(M)=\det(E_1)\det(E_2)\det(E_3)=(1)(-5)(1)=-5,
	\]
	which is exactly what we got from the formula.
\end{example}
...
\begin{example}
	Use the diagonal trick to compue $\det\left(\mat{1 & 4 & 0 \\ -2 & 3 & 1 \\ 0 & 2 & 1}\right)$.
...
\begin{example}
	...
	Let $M=\mat{1 & -3 \\ 2 & 2}$. Since $\det(M)=(1)(2)-(2)(-3)=8>0$, we conclude that $\Set*{\mat{1\\2},\mat{-3\\2}}$ is right-handed.
\end{example}
...
We can compute the determinant of the matrix $Q=[\xhat|\,\vec u_\theta]$ directly using the $2\times 2$ determinant formula.
\[
	\det(Q) = \det([\xhat|\,\vec u_\theta]) = \det\left(\mat{1&\cos\theta\\0&\sin\theta}\right) = \sin\theta.
\]

Visually, we know that $\mathcal Q$ is right-handed when $\theta\in(0,\pi)$, left handed when $\theta\in(\pi,2\pi)$ and $\mathcal Q$
is not a basis when $\theta=0$ or $\theta =\pi$. And, the determinant corroborates this. $\det(Q)=\sin\theta>0$ when $\theta\in(0,\pi)$,
$\det(Q)=\sin\theta < 0$ when $\theta\in(\pi,2\pi)$ and $\det(Q)=\sin\theta=0$ when $\theta\in\Set{0,\pi}$.
