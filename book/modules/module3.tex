Let $\vec u=\mat{1\\1}$ and $\vec v=\mat{1\\-2}$. Can the vector $\vec w=\mat{2\\5}$ be obtained
...
\begin{center}
	\begin{tikzpicture}
		\begin{axis}[
			...
			\draw[myorange, thick] (0,1) circle[radius=1cm] node[right] {$X$};
		\end{axis}
	\end{tikzpicture}
	~~~~
	\begin{tikzpicture}
		\begin{axis}[
			...
			\draw[mypink, thick] (0,1) circle[radius=1cm] (3,0) circle[radius=1cm] node[above right] {$Y$};
		\end{axis}
	\end{tikzpicture}
	~~~~
	\begin{tikzpicture}
		\begin{axis}[
			...
			\draw[mygreen, thick] (0,0) circle[radius=1cm] (0,1) circle[radius=1cm] (1,0) circle[radius=1cm] node[above right] {$Z$};
		\end{axis}
	\end{tikzpicture}
\end{center}
...
\begin{center}
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			\fill[black!60!white] (2,2.5) circle[radius=1.5pt];
...
\begin{example}
	...
	We could answer this question using techniques from Module \ref{MODULELINESANDPLANES}, but for variety,
	let's see if we can answer the question using spans and linear dependence.

	Let $\vec a_1=\mat{1\\2\\1}$ and $\vec a_2=\mat{2\\2\\1}$ be direction vectors
	for $\mathcal P$ and let
	$\vec b_1=\mat{3\\4\\2}$ and $\vec b_2=\mat{2\\2\\1}$ be direction vectors
	for $\mathcal Q$ and notice $\mathcal P=\Span\Set{\vec a_1,\vec a_2}$ and $\mathcal Q=\Span\Set{\vec b_1,\vec b_2}$.

	By definition $\mathcal P=\mathcal Q$ if (i) every point in $\mathcal P$ is a point in $\mathcal Q$ and (ii) every point in $\mathcal Q$
	is a point in $\mathcal P$.

	Focusing on (i), let $\vec p=t\vec a_1+s\vec a_2\in \mathcal P$ be an arbitrary point in $\mathcal P$. We need to show
	$\vec p\in\mathcal Q$.
	Since $\Set{\vec b_1,\vec b_2}$ is linearly independent and $\mathcal Q=\Span\Set{\vec b_1,\vec b_2}$, showing
	$\vec p\in\mathcal Q$ is equivalent to showing $\Set{\vec p,\vec b_1,\vec b_2}$ is a linearly \emph{dependent} set.

	To see this, start by observing
	\[
		\Set{\vec a_1,\vec b_1,\vec b_2}\qquad\text{and}\qquad \Set{\vec a_2,\vec b_1,\vec b_2}
	\]
	are both linearly dependent sets: $\vec a_1=\vec b_1-\vec b_2\in\Span\Set{\vec b_1,\vec b_2}$
	and $\vec a_2=\vec b_2\in\Span\Set{\vec b_1,\vec b_2}$. Therefore, $\vec a_1\in\mathcal Q$ and
	$\vec a_2\in\mathcal Q$. Since both $\vec a_1$ and $\vec a_2$ are in $\mathcal Q$ and $\mathcal Q=\Span\Set{\vec b_1,
	\vec b_2}$ is itself a span, we know that every linear combination of $\vec a_1$ and $\vec a_2$ must be in $\mathcal Q$.
	In particular, $\vec p=t\vec a_1+s\vec a_2\in\mathcal Q$, which is what we wanted to show.

	We can show (ii) similarly by observing that $\vec b_1\in\mathcal P$ and $\vec b_2\in \mathcal P$ and so any
	point $\vec q=t\vec b_1+s\vec b_2\in\mathcal Q$ must also be in $\mathcal P$.
\end{example}
