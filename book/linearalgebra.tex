...
\begin{module}
	...
	\begin{itemize}
		...
		\item[] Quick Check: Let $\mathbb{L}$ be the set of English letters. Let $S=\Set
			{e,\ell,a}$. Write down a subset $T_{1}$ of $\mathbb{T}$ so that \emph{some}
			elements of $S$ are in $T$. Write down another subset $T_{2}$ of
			$\mathbb{T}$ so that \emph{all} elements of $S$ are in $T_{2}$.
...
\begin{module}
	...
	\begin{itemize}
		...
		\item[] Qhich check: Solve the system of linear equations
			$\systeme{x+2y=7, -x+3y=8}$.
	
		\item Working with lines and planes
	
		\item[] Let $\mathcal{L}$ be the line passing through $(1,0)$ and $(2,4)$.
...
\begin{module}
	...
	\begin{itemize}
		...
		\item[] Express the line passing through $(2,3)$ and $(4,6)$ as a span or a translated
			span.
	\end{itemize}
...
\begin{module}
	...
	\begin{itemize}
		...
		\item[] Let $\mathcal{L}$ be given by $\mat{3 \\ 2}\cdots (\vec x-\mat{4 \\ 7}
