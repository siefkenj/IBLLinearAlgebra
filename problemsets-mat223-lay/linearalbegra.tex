...
\subsection*{$QR$ Decomposition}
	...
	\begin{definition}[Vector Space]
		...
		if it satisfies the following axioms for all $\vec a,\vec b,\vec c\in V$
		and all $\alpha,\beta\in\R$.
		\begin{enumerate}
			\item $(\vec a+\vec b)+\vec c=\vec a+(\vec b+\vec c)$
			\item $\vec a+\vec b=\vec b+\vec a$
			\item There is an element $\vec 0\in V$ so that $\vec a+\vec 0=\vec a$
			\item There is an element $-\vec a\in V$ sot hat $\vec a+(-\vec a)=\vec 0$
			\item $\alpha\cdot (\vec a+\vec b)=\alpha\cdot \vec a+\alpha\cdot \vec b$
			\item $(\alpha+\beta)\cdot\vec a = \alpha\cdot \vec a+\beta\cdot \vec a$
			\item $\alpha\cdot (\beta \cdot \vec a) = (\alpha\beta)\cdot \vec a$
	...
	\begin{definition}[Span]
		...
		of $X$ to be the set
		\[
			\Span X = \{\vec x\in V:\text{$\vec x$ is a linear combination of vectors in $V$}\}.
		\]
		We define
		\[
			\Span \{\} = \{\vec 0\}.
