...
\subsection*{$QR$ Decomposition}
	...
	\question
		Let $T:\mathcal V\to\mathcal W$ be a linear transformation and let $\mathcal U\subseteq \mathcal W$
		be a subspace.
	\begin{parts}
	...
	\question
	Let $\vec u=\mat{1\\0\\0}$ and $\vec v=\mat{1\\1\\0}$ and $\mathscr B=\{\vec u,\vec v\}$. Define $\mathcal V=\Span \mathscr B\subseteq \R^3$ 
	and consider
	...
	\]
	\begin{parts}
		\item Is $\vec x=\mat{2\\1\\0}$ in the domain of $T$? If so, compute $T(\vec x)$.
		\item Is $\vec y=\mat{2\\1\\1}$ in the domain of $T$? If so, compute $T(\vec y)$.
		\item Write $[\vec x]_{\mathscr B}$ and $[\vec y]_{\mathscr B}$, if possible.
		\item Is $T$ invertible?
		\item What is $[T]_{\mathscr B}^{\mathscr B}$? Does $[T]_{\mathscr B}^{\mathscr B}=\mat{1&0&0\\0&1&0}$?
			Is there another basis $\mathscr C$ so that $[T]_{\mathscr C}^{\mathscr C}=\mat{1&0&0\\0&1&0}$?
			Why or why not?
	...
	\question
		Let $\mathscr A$ and $\mathscr B$ be bases for $\R^2$.
			...
			$M_A,M_B:\R^2\to\R^2$ defined by multiplication by the matrices $A=[T]_{\mathscr A}$
			and $B=[T]_{\mathscr B}$.
	\begin{parts}
	...
	\]
	\[
		\mathscr V(T)=\text{ $1$ if $\mat{1\\1}$ is an eigenvector for $T$ and $0$ otherwise}
	\]\[
		\mathscr E(T)=\text{ $1$ if $3$ is an eigenvvalue for $T$ and $0$ otherwise}
	\]
	\begin{parts}
