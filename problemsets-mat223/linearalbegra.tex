	...
	\begin{parts}
		\item The rank of a $3\times 4$ matrix $A$ is $3$. 
			...
			\begin{solution}
				...
				in $\R^3$ to be linearly indepedent (and if any two of the columns
				are equal, they are linearly dependent).
			\end{solution}
		\item The rank of a $4\times 3$ matrix $B$ is $3$.
			Are the column vectors of $B$ linearly independent?
			\begin{solution}
				Yes. The rank of $B$ is the number of pivots in $\rref(B)$, which
				we know is equal to the dimension of the column space of $B$. In
				this case, since there are only three columns, if the dimension
				of their span is 3, they must all be linearly independent. 
			\end{solution}
	\end{parts}
	...
	\begin{parts}
		\item What is the rank and nullity of $A^T$?
			\begin{solution}
				...
				$A^T$ is the matrix with $\vec u,\vec v$, and $\vec w$ as its 
				respective rows. Since $\vec w$ is a linear combination of 
				$\vec u$ and $\vec v$, the third row of $A^T$ can be reduced to 
				a row of zeros by elementary row operations. Neither of the first
				two rows can be reduced to rows of zeros since they are linearly
				independent. Therefore $\rref(A^T)$ has two pivots, meaning
				$\Rank(A^T)=2$. 
