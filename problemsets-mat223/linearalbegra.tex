...
\section*{Matrices}
	...
	\begin{parts}
		...
		\item Let $\mat{x_0\\y_0}$ be a solution to $A\vec x=\vec b$. Explain what
			...
			\begin{solution}
				The lines represented by the equations in the system above intersect
				at the point $\mat{x_0\\y_0}$.
			\end{solution}
	\end{parts}
	...
	\begin{parts}
		\item How could you determine if $\Set{\vec u,\vec v,\vec w}$ was a linearly
			independent set?
			\begin{solution}
				The set is is linearly independent if and only if the only linear
				combination	of them that equals $\vec 0$ is the trivial one. That
				is, if $x, y, z$ are scalars such that $x\vec u+y\vec v+z\vec w = \vec 0$,
				then $x=y=z=0$.
