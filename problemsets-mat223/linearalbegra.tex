	...
	\begin{parts}
		...
		\item Can you find a matrix $X$ so that $X[\vec w]_{\mathcal C} = [\vec w]_{\mathcal E}$
			...
			\begin{solution}
				...
				We know from the first part above that
				\[
					\mat{1\\0}_{\mathcal C} = \mat{2\\1}_{\mathcal E}
					\quad \text{and} \quad
					\mat{0\\1}_{\mathcal C} = \mat{5\\3}_{\mathcal E},
				\]
				and that we need $X$ to satisfy
				\[
					X\mat{1\\0}_{\mathcal C}=\mat{a\\c}=\mat{2\\1}_{\mathcal E}
					\quad \text{and} \quad
					X\mat{0\\1}_{\mathcal C}=\mat{b\\d}=\mat{5\\3}_{\mathcal E}.
				\]
				We can now immediately solve for $a,b,c,d$ to find that $X$ must
				be the matrix $\mat{2&5\\1&3}$.
		...
		\item Can you find a matrix $Y$ so that $Y[\vec w]_{\mathcal E} = [\vec w]_{\mathcal C}$
			...
			\begin{solution}
				...
				We know from the second part above that
				\[
					\mat{1\\0}_{\mathcal E} = \mat{3\\-1}_{\mathcal C}
					\quad \text{and} \quad
					\mat{0\\1}_{\mathcal E} = \mat{-5\\2}_{\mathcal C},
				\]
				and that we need $Y$ to satisfy
				\[
					Y\mat{1\\0}_{\mathcal E}=\mat{a\\c}=\mat{3\\-1}_{\mathcal C}
					\quad \text{and} \quad
					Y\mat{0\\1}_{\mathcal E}=\mat{a\\c}=\mat{-5\\2}_{\mathcal C},
				\]
				We can now immediately solve for $a,b,c,d$ to find that $Y$ must
				be the matrix $\mat{3&-5\\-1&2}$.
			\end{solution}
		\item What is $YX$?
			\begin{solution}
				$YX = \mat{1&0\\0&1}$.

				We should expect this since for a vector $\vec w$, we should have:
				\[
					YX[\vec w]_{\mathcal C}=Y[\vec w]_{\mathcal E}=[\vec w]_{\mathcal C}.
				\]
				In other words, $YX$ should not change any vector to which it is applied.
			\end{solution}
	\end{parts}
