...
\end{lesson}
	...
	\begin{parts}
		\item What is $A^{-1}A$?
			\begin{solution}
				$A^{-1}A = I$. This is true by definition of an inverse, but we
				can also verify it by hand.
			\end{solution}
		\item What is $\rref(A)$?
			\begin{solution}
	...
	\begin{parts}
		\item For two square matrices $X,Y$, should $(XY)^{-1}=X^{-1}Y^{-1}$?
			\begin{solution}
				...
				By the definition of an inverse, we should want $(XY)(XY)^{-1}=I$. 
				But there is no reason for $(XY)(X^{-1}Y^{-1})$ to equal the
				identity, since we cannot in general rearrange the order of product
				of matrices.

				In fact, we should expect $(XY)^{-1} = Y^{-1}X^{-1}$, since
				\[
		...
		\item If $M$ is a matrix corresponding to a non-invertible linear transformation $T$,
			...
			\begin{solution}
				...
				If $M$ were invertible, then its inverse $M^{-1}$ would be a matrix
				corresponding to the inverse of the transformation $T$, which is
				impossible if $T$ is non-invertible. 
			\end{solution}
	\end{parts}
