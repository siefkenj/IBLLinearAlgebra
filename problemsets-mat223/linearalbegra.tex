...
\section*{Lines and Planes}
	...
	\begin{parts}
		\item Describe $A$ using set-builder notation.
			\begin{solution}
				$\Set*{\vec v \in \R^2 \given \vec v = \begin{bmatrix*}[c] x \\ 2x+1\end{bmatrix*} \text{ for some } x \in \R}$
				or
				$\Set*{ \mat{x \\ y} \in \R^2 \given y = 2x+1 }$
				or
				$\Set*{  \begin{bmatrix*}[c] x \\ 2x+1\end{bmatrix*} \in \R^2 \given x \in \R}$
			\end{solution}
...
\end{lesson}
	...
	\begin{parts}
		\item Write $\ell$ in vector form. Is vector form of $\ell$ unique?
			\begin{solution}
				...
				The vector form is not unique, as any scalar multiple of the 
				direction vector will work in place of $\mat{1 \\ -2}$, as will
				any other point the line passes through work in place of 
		...
		\item Find another vector form for $L$ where both ``$\vec d$'' and
			...
			\begin{solution}
				Again, any multiple of the direction vector will work for 
				$\vec d$, as will any other point on the line work for 
				$\vec p$. For example, this equation describes the same 
				line: $\vec x = t \mat{-3 \\ 6 \\ 6} + \mat{1 \\ 1 \\ 1}$.
			\end{solution}
	\end{parts}
	...
	\begin{parts}
		\item Do the lines $A$ and $B$ intersect? Justify your conclusion.
			\begin{solution}
				Yes. In this case this is quite easy to see since 
				$\mat{0 \\ 0 \\ 1}$ is a point on line $A$, and this same point
				 is on line $B$ (for the value $t = -1$).
			\end{solution}
		\item Do the lines $A$ and $C$ intersect? Justify your conclusion.
			\begin{solution}
				No. In order for them to intersect, there would have to exist 
				two real numbers $a$ and $c$ such that when substituded for $t$ 
				in the equations of $A$ and $C$, respectively, the resulting 
				vectors would equal. In other words, we would have to have:
				\[
					a\mat{1\\2\\3}+\mat{0\\0\\1} = c\mat{2\\-1\\1}+\mat{1\\1\\1}.
				\]
				This amounts to showing that the following system of equations 
				has no solutions:
				\begin{gather*}
					a = 2c + 1 \\
					2a = -c + 1 \\
					3a + 1 = c + 1
				\end{gather*}
				The third equation tells us that $c = 3a$, which when substituted 
				into the first equation forces $a = -\tfrac{1}{5}$ and therefore
				$c = -\tfrac{3}{5}$. These two numbers don't satisfy the second
				equation, however.
			\end{solution}
		\item Let $\vec p\neq \vec q$ and suppose
			...
			\begin{solution}
				Yes. This can happen for example if $\vec q$ is a point, other 
				than $\vec p$, on the line $\vec x = t \vec d + \vec p$.

				One example might be the lines
				\[
					\vec x = t \mat{1\\1\\1} + \mat{0\\0\\0} 
					\qquad \text{and} \qquad
					\vec x = t \mat{1\\1\\1} + \mat{7\\7\\7}.
				\]
				These two are in fact the same line, so they intersect everywhere.
			\end{solution}
	\end{parts}
...
\end{lesson}
	...
	\begin{parts}
		\item Find two direction vectors in $\mathcal P$.
			\begin{solution}
				The easiest ones to find are the two direction vectors we 
				already know are in the plane---the ones from the two lines:
				\[
					\vec d_1 = \mat{1\\2\\3}
					\qquad \text{and} \qquad
					\vec d_2 = \mat{-1\\1\\1}.
		...
		\item Write $\mathcal P$ in vector form.
			\begin{solution}
				We already have two direction vectors, so we just need a point
				on the plane. We will use the point $\vec p = \mat{0\\0\\1}$ 
				that we already know is on line $A$. This gives the vector 
				equation:
				\[
					\vec x = t\vec d_1 +s\vec d_2+\vec p
		...
		\item Write $\mathcal P$ in vector form using different
			...
			\begin{solution}
				...
				scalar multiple of either direction	vector and get the same 
				plane. We can also use the point $\vec q = \mat{-1\\1\\2}$ 
				that we already know is on line $B$. Here's one possibility:
				\[
...
\end{lesson}
	...
	\begin{parts}
		\item If possible, describe $\ell_1$ as a span. Otherwise explain why 
			it's not possible.
			\begin{solution}
				$\ell_1$ is the span of the vector $\mat{1\\1}$, since a point 
				$\mat{1\\1}$ is on this line of and only if $x = y$, which in 
				turn is true if and only if it is a scalar multiple of $\mat{1\\1}$. 
			\end{solution}
		\item If possible, describe $\ell_2$ as a span. Otherwise explain why it's not 
			possible.
			\begin{solution}
				This is not possible. The origin $\mat{0\\0}$ must always be in the span
				of any set of vectors, since we can use all zeroes as the scalars in a 
				linear combination.	But the point $\mat{0\\0}$ is not on $\ell_2$, so it 
				cannot be described	as a span.
			\end{solution}
		\item Does the expression $\Span(\ell_1)$ make sense? If so, what is it? How about
			...
			\begin{solution}
				...
				of any set of vectors, and these lines are just special set of points in $\R^2$
				which we are already used to thinking of as vectors.

				$\Span(\ell_1)$ just equals $\ell_1$, since all of the vectors on the line 
				$\ell_1$ are already multiples of $\mat{1\\1}$, as we discovered earlier. 

				$\Span(\ell_2)$ equals all of $\R^2$. It's easy to see that the vectors 
				$v = \mat{4\\0}$ and $w = \mat{0\\-4}$ are both on $\ell_2$, and the span
				of these two vectors alone is all of $\R^2$. 
			\end{solution}
	\end{parts}
	...
	\begin{parts}
		...
		\item Is $A+B$ the same as $B+A$?
			\begin{solution}
				Yes. Since $A$ and $B$ are such small sets we could easily just
				compute all the vectors in $A+B$ and $B+A$ and see that they're 
				equal. However, we know from elementary school that real numbers 
				can be added up in any order, and the coordinates of an element of 
				$A+B$ or $B+A$ are simply sums of the corresponding coordinates of 
				elements of $A$ and $B$. 
			\end{solution}
		\item Draw $\ell+A$.
...
\pagestyle{siefken}
	...
	\begin{parts}
		...
		\item Is $\{\vec u,\vec v,\vec w\}$ linearly independent? Why or why not?
			\begin{solution}
				No. It's easy to see that $\vec w=\vec u+\vec v$, and so
				$\vec w \in \Span\Set{\vec u, \vec v}$.
			\end{solution}
	\end{parts}
	...
	\begin{parts}[resume]
		\item Give a subset $Y\subseteq X$ so that $\Span Y=\Span X$ and $Y$ is
			...
			\begin{solution}
				One $Y$ that works is $Y = \Set{\vec u,\vec v}$. 
			\end{solution}
