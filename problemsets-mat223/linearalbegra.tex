...
\subsection*{More Change of Basis}
	...
	\begin{parts}
		...
		\item Compute $X[\xhat]_{\mathcal B}$ and $X[\yhat]_{\mathcal B}$.  
			What do you notice?
			\begin{solution}
				$X[\xhat]_{\mathcal B}=\frac{1}{2}\mat{1&1\\1&-1}\mat{1\\1}=\mat{1\\0}$
				and 
				$X[\yhat]_{\mathcal B}=\frac{1}{2}\mat{1&1\\1&-1}\mat{1\\-1}=\mat{0\\1}$

				We notice that multiplying by $X$ seems to turn the representations
				of these two vectors in the basis $\mathcal B$ into representations
	...
	\begin{parts}
		\item Should $X^{-1}$ exist? Explain.
			\begin{solution}
				...
				$\mathcal B$ basis, and this process be undone. $X^{-1}$ should
				be the matrix that does this.
			\end{solution}
