	...
	\begin{parts}
		\item Find a matrix $P$ so that $P\vec x=\mathcal P(\vec x)$ for all $\vec x\in\R^2$.
			\begin{solution}
				...
				So if $\Set{\xhat,\yhat}$ is the standard basis for $\R^2$,
				we can compute
				\[
					\mathcal P(\xhat)=\frac{2}{13}\vec u=\frac{1}{13}\mat{4\\6}
					\qquad \text{and} \qquad
					\mathcal P(\yhat)=\frac{3}{13}\vec u=\frac{1}{13}\mat{6\\9}.
				\]
				So since we know $\mathcal P$ is linear, for any $\vec x=\mat{x\\y}$,
				we can compute:
				\[
					\mathcal P(\vec x)
					=x\mathcal P(\xhat)+y\mathcal P(\yhat)
		...
		\item Write down matrices $A$ and $B$ for $\mathcal P\circ\mathcal R$ 
			...
			\begin{solution}
				...
				Using the same reasoning as part 1 above, if $\Set{\xhat,\yhat}$
				is the standard basis for $\R^2$, we can compute
				\[
