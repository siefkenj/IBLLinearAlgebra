	...
	\begin{parts}
		...
		\item Draw $\mathcal R(S)$, $T(S)$, and $\mathcal P(S)$ (where $\mathcal R$,
			...
			\begin{solution}
				XXXX
			\end{solution}
		\item Suppose that $\ell=\Set{\text{all convex combinations of }\vec a\text{ and }\vec b}$
			is a line segment with endpoints $\vec a$ and $\vec b$ and $A$ is a 
			linear transformation. Must $A(\ell)$ be a line segment? 
			What are its endpoints?
			\begin{solution}
				Yes, $A(\ell)$ must be a line segment.
				Its endpoints are $A(\vec a)$ and $A(\vec b)$.

				If $\vec x=\alpha_1\vec a+\alpha_2\vec b$ is a convex combination
				from $\ell$, then $A(\vec x)=\alpha_1A(\vec a)+\alpha_2A(\vec b)$,
				which is a convex combination of $A(\vec a)$ and $A(\vec b)$.
			\end{solution}
