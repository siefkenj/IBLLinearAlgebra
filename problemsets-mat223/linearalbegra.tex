...
\end{center}
	...
	\question
	Let $\mathcal E=\Set{\vec e_1,\vec e_2}$ be the standard basis for $\R^2$ and 
	...
	\begin{parts}
		\item Express $\vec c_1$ and $\vec c_2$ as a linear combination of $\vec e_1$ and $\vec e_2$.
			\begin{solution}[inline]
				$\vec c_1=2\vec e_1+\vec e_2$ and $\vec c_2=5\vec e_1+3\vec e_2$.
			\end{solution}
		\item Express $\vec e_1$ and $\vec e_2$ as a linear combination of $\vec c_1$ and $\vec c_2$.
			\begin{solution}[inline]
				$\vec e_1=3\vec c_1-\vec c_2$ and $\vec e_2=-5\vec c_1+2\vec c_2$.
			\end{solution}
...
\end{lesson}
	...
	\question
	Let $\Set{\vec e_1,\vec e_2}$ be the standard basis for $\R^2$ and let
	$\vec u_{\theta}$ be a unit vector. Let $\theta$ be the angle between $\vec u_{\theta}$ and
	$\vec e_1$ measured counter clockwise starting at $\vec e_1$.
	\begin{parts}
		\item For which $\theta$ is $\Set{\vec e_1,\vec u_{\theta}}$ a linearly independent set?
			\begin{solution}[inline]
				Every $\theta$ that is not a multiple of $\pi$. 
			\end{solution}
		\item For which $\theta$ can $\Set{\vec e_1,\vec u_{\theta}}$ be continuously
			transformed into $\Set{\vec e_1,\vec e_2}$ and remain linearly independent the
			whole time?
			\begin{solution}
				Every $\theta \in (0, \pi)$. 

				For $\theta\in(\pi,2\pi)$, a continuous transformation of 
				$\vec u_\theta$	to $\vec e_2$ would have to cross the $x$-axis,
				at which point $\Set{\vec e_1,\vec u_\theta}$ would cease to be 
				linearly independent. 
			\end{solution}
		\item For which $\theta$ is $\Set{\vec e_1,\vec u_{\theta}}$ right-handed?	
			Left-handed?
			\begin{solution}
				It is right-handed for $\theta\in(0, \pi)$, and left handed for
				$\theta\in(\pi,2\pi)$.
			\end{solution}
		\item For which $\theta$ is $\Set{\vec u_{\theta},\vec e_1}$ (in that order) 
...
\end{lesson}
	...
	square and let $C=\Set{\vec 0, \vec e_1,\vec e_2,\vec e_1+\vec e_2}\subseteq \R^2$
	be the corners of the unit square.
	\begin{parts}
		\item Find $\mathcal R(C)$, $W(C)$, and $T(C)$ (where $\mathcal R$, $W$,
			and $T$ are from the previous question).
			\begin{solution}
				$\mathcal R(C) = \Set{\vec 0,\vec e_2,-\vec e_1,-\vec e_1+\vec e_2}$.
...
\end{lesson}
	...
	\begin{parts}
		\item What is the difference between	
			...
			\begin{solution}
				...
				different bases of $\R^2$, and so a given vector $\vec v$ might
				have infinitely many different representations as a column vector,
	...
	\begin{definition}[Fundamental Subspaces]
		...
		the \emph{row space}, denoted $\Row(M)$, of $M$ is the span of the rows of $M$; 
		the \emph{column space}, denoted $\Col(M)$, of $M$ is the span of the columns of $M$;
		and the \emph{null space}, denoted $\Null(M)$, of $M$ is the set of solutions to $M\vec x=\vec 0$. 
	\end{definition}
	...
	\begin{parts}
		...
		\item Describe the range and null space of $T_A$, the transformation
			induced by $A$.
			\begin{solution}
				$\Range(T_A)=\Set{\vec v\in\R^2 \given [\vec v]_{\mathcal E}\in\Col(A)}=\R^2$	
				and $\Null(T_A)=\Set{\vec v\in\R^3 \given [\vec v]_{\mathcal E}\in\Null(A)}$, 
				which is the $z$-axis in $\R^3$. 

				First, by definition, $\vec v\in\Range(T_A)$ when 
				$[\vec v]_{\mathcal E}=[T_A\vec x]_{\mathcal E}=A[\vec x]_{\mathcal E}$,
				for some $\vec x\in\R^3$. But for any vector $\vec x\in\R^3$, if 
				$[\vec x]_{\mathcal E}=\mat{x\\y\\z}$, we know that 
				\[
					A[\vec x]_{\mathcal E} = x\mat{1\\0}+y\mat{0\\1}.
				\]
				In other words $A[\vec x]_{\mathcal E}$ can be anything in $\Col(A)$,
				and so $\Range(T_A)$ is the set of all vectors whose representation
				in the standard basis is in $\Col(A)$. We know from a previous part
				that $\Col(A)=\R^2$, and so any vector's representation in the
				standard basis is in $\Col(A)$. 
	...
	\begin{parts}
		\item How does the row space of $B$ relate to the row space of $C$?
			\begin{solution}
				...
				can be obtained from $C$ by applying row operations, and so 
				$\Row(B)\subseteq\Row(C)$.

				Therefore, the two spans are equal. 
			\end{solution}
		\item How does the null space of $B$ relate to the null space of $C$?
			\begin{solution}
				...
				respectively. But the $\Row(B)=\Row(C)$ by the previous part, and 
				so their null spaces must also be equal. 
			\end{solution}
		\item Compute the null space of $B$.
			\begin{solution}
				...
				\[
					C\vec x = \mat{x-z\\y+2z} = \vec 0.
				\]
				This is true if $x=z$ and $y=-\frac{z}{2}$, so the null space is
				\[
					\Null(C)=
