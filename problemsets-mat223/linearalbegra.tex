...
\section*{Matrices}
		...
		\item If $\vec b=\mat{1\\1\\0}$, is the set of solutions to this system
			...
			\begin{solution}
				...
				\begin{align*}
					...
					0 &= 0
				\end{align*}
				$\vec x=\mat{x\\y\\z}$ satisfies this system if and only if it is
				on the line $\vec x = t \mat{3\\1\\0} + \mat{1\\0\\1}$.
			\end{solution}
	\end{parts}
	...
	\begin{parts}
		...
		\item Find such an $M$.
			\begin{solution}
				$M=\mat{1&-1&0\\1&1&0\\2&0&0}$ is one possible answer, since if
				$\vec r = \mat{a\\b\\c}$, then $M \vec r = a \vec d_1 + b \vec d_2$. 

				Another less interesting answer is the zero matrix. 
			\end{solution}
		...
		\item Find a matrix $K$ so that solutions to $K\vec x=\vec 0$ are normal
			...
			\begin{solution}
				$K = \mat{1&1&2\\-1&1&0\\0&0&0}$. $K$ and $M$ are transposes of 
				one another.

				The conditions in the previous part translate to the following 
				system of equations:
				\begin{align*}
					x + y + 2z &= 0 \\
					-x + y &= 0.
				...
				\[
					\mat{1&1&2\\-1&1&0\\0&0&0}\mat{x\\y\\z} = \mat{0\\0\\0}.
				\]
			\end{solution}
	\end{parts}
