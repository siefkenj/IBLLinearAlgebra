...
\section*{Inverses}
	...
	\begin{parts}
		...
		matrix and call the result $E_1$.
			\begin{solution}	
				$\mat{1&0&0\\0&1&0\\0&0&1} \xrightarrow{R_3\to R_3+2R_1} \mat{1&0&0\\0&1&0\\2&0&1}=E_1$.
			\end{solution}	
		\item Apply the row operation $R_3\to R_3-2R_1$ to the $3\times 3$ identity
		matrix and call the result $E_2$.
			\begin{solution}	
				$\mat{1&0&0\\0&1&0\\0&0&1} \xrightarrow{R_3\to R_3-2R_1} \mat{1&0&0\\0&1&0\\-2&0&1}=E_2$.
			\end{solution}	
	\end{parts}
	...
	\begin{parts}[resume]
		...
		operations?
			\begin{solution}
				...
				Both of these matrices are the result of applying to $A$ the same
				row operation that was applied to the respective $E_1$ or $E_2$. 

				That is, $E_1A$ is the result applying the row operation $R_3\to R_3+2R_1$
				to $A$, and similarly $E_2A$ is the result of applying the row
				operation $R_3\to R_3-2R_1$ to $A$. 
			\end{solution}
		...
		operation $R_3\to R_3-2R_1$ to $E_1$ be? Compute and verify.
			\begin{solution}
				...
				$R_3\to R_3-2R_1$ should undo the operation $R_3\to R_3+2R_1$ that
				turned the identity matrix into $E_1$. 
			\end{solution}
		...
		Now compute and verify.
			\begin{solution}
				...
				to a matrix has the effect of applying the row operation $R_3\to R_3+2R_1$
	...
	\begin{parts}
		\item Which pairs of matrices above are inverses of each other?
			\begin{solution}
				$A$ and $D$ are inverses of each other, and $B$ and $C$ are 
				inverses of each other. These are the only such pairs. 
			\end{solution}
	\end{parts}
	...
	\begin{parts}
		\item Use two row operations to reduce $B$ to $I_{2\times 2}$ and write
			...
			\begin{solution}
				$\mat{1&4\\0&2}\xrightarrow{R_2\to \frac{1}{2}R_2}\mat{1&4\\0&1}
				\xrightarrow{R_1\to R_1-4R_2}\mat{1&0\\0&1}$.
		...
		\item Find $B^{-1}$.
			\begin{solution}
				...
				By previous part we already know that $(E_2E_1)B=I$. We can also
		...
		using elementary matrices?
			\begin{solution}
				...
				Then $E_n E_{n-1} \cdots E_2 E_1$ is the inverse of $A$. 
			\end{solution}
	\end{parts}
