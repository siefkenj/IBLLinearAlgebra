...
\end{lesson}
	...
	\begin{parts}
		...
		\item If $\vec x=\mat{x\\y}$ and $\vec x$ is orthogonal to $\vec u$, 
			...
			\begin{solution}[inline]
				$\vec x\cdot \vec u = 0$, by definition of orthogonality.
			\end{solution}
		\item Expand the dot product $\vec u\cdot \vec x$ to get an equation for $A$.
			\begin{solution}
				$A$ is the line with vector equation $\vec x = t\mat{3\\2}$. 

				If $\vec x=\mat{x\\y} \in A$, then we have:
				\[
					\vec x\cdot\vec u = 2x + 3y = 0.
				\]
				The vector $\mat{3\\2}$ satisfies this equation, as does any
				other vector if and only if it is a multiple of this one.
			\end{solution}
