...
\end{lesson}
	...
	\begin{parts}
		\item Use set-builder notation to write down the set, $X$, of
			...
			geometrically.
			\begin{solution}
				$X = \Set*{\vec x\in \R^2 \given \vec x\cdot \vec n = 0}$.

				Geometrically, this is a line through the origin and 
				perpendicular to $\vec n$. 
			\end{solution}
		\item Describe $X$ using an equation.
			\begin{solution}
				$\vec x = t\mat{1\\-2\\1}$ is one way to do this with a vector equation.
			\end{solution}
