	...
	\begin{parts}
		...
		\item Can you find a matrix $X$ so that $X[\vec w]_{\mathcal C} = [\vec w]_{\mathcal E}$
			...
			\begin{solution}
				...
				For a vector $\vec w$, if $[\vec w]_{\mathcal C}=\mat{x\\y}$ then
				we know $\vec w=x\vec c_1+y\vec c_2$. 

				From the result of part 1 above, we then have
				\[
					\vec w
					=x(2\vec e_1+\vec e_2) + y(5\vec e_1+3\vec e_2)
					=(2x+5y)\vec e_1 + (x+3y)\vec e_2.
				\]
				In other words, 
				\[
					[\vec w]_{\mathcal E}
		...
		\item Can you find a matrix $Y$ so that $Y[\vec w]_{\mathcal E} = [\vec w]_{\mathcal C}$
			...
			\begin{solution}
				...
				Using similar reasoning to the previous part and the result from
				part 2 above, if $[\vec w]_{\mathcal E}=\mat{x\\y}$ then we know
				\[
					\vec w
					=x(3\vec c_1-\vec c_2)+y(-5\vec c_1+2\vec c_2)
					=(3x-5y)\vec c_1 + (-x+2y)\vec c_2.
				\]
				In other words,
				\[
					[\vec w]_{\mathcal C}
