		...
		\item For what values of $\lambda$ does $E_\lambda$ have a non-trivial
			...
			\begin{solution}
				...
				\[
					...
					=(\lambda-1)(\lambda+2).
				\]
				This equals zero exactly when $\lambda=-2$ and $\lambda=1$.
			\end{solution}
	...
	\begin{parts}
		...
		\item What is $\Nullity(E)$, $\Nullity(E-3I)$, $\Nullity(E+3I)$?
			\begin{solution}
				...
				$E$ has three eigenvalues. Each one corresponds to a set of
				eigenvectors that is one of the null spaces in the problem.
				For example, $E$ has an eigenvector $\vec v$ corresponding to the 
				eigenvalue $0$, and $\vec v\in\Null(E_0)=\Null(E)$, as is any 
				scalar multiple of $\vec v$. 

				So these three null spaces all have dimension at least 1, and 
				they are all linearly independent meaning none of them can have
				dimension greater than 1 since they are subspaces of $\R^3$. 
			\end{solution}
	\end{parts}
