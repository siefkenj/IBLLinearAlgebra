...
\section*{Dot Product}
	...
	\begin{parts}
		...
		\item Suppose $\vec y$ is a unit vector and the distance between $\vec y$ 
			...
			\begin{solution}
				The angle between them is $\arccos\left(-\frac{3}{2\sqrt{6}}\right)$. 

				\

				From the theorem above, we know that $2=\|\vec u-\vec y\|=\sqrt{(\vec u-\vec y)\cdot(\vec u-\vec y)}$.

				Squaring both sides and distributing the dot product, we obtain
				\[
					4 = \vec u\cdot\vec u - 2\vec u\cdot\vec y + \vec y\cdot\vec y
					= \|\vec u\|^2 - 2\vec u\cdot\vec y + \|\vec y\|^2
					= 6 - 2\vec u\cdot\vec y + 1,
				\]
				and rearrange to find that $\vec u\cdot \vec y = -\frac{3}{2}$.
