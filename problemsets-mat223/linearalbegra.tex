...
\end{lesson}
	...
	\begin{parts}
		...
		\item Describe how you can use $P$ and $P^{-1}$ to easily compute
			...
			\begin{solution}
				The key observation is that $A$ acts in a very simple way on 
				vectors represented in the $\mathcal V$ basis, as we found in the
				previous problem. In other words, for any vector $\vec v$, it is
				simple to compute $A[\vec v]_{\mathcal V}$. We also know from the
				previous part that $P^{-1}[\vec v]_{\mathcal E}=[\vec v]_{\mathcal V}$,
				where $\mathcal E$ is the standard basis. 

				So given any vector $\vec v$ represented by $[\vec v]_{\mathcal E}$
				in the standard basis, we have
				\[
					[A\vec v]_{\mathcal E}=PAP^{-1}[\vec v]_{\mathcal E}
				\]
				since
				\[
					PAP^{-1}[\vec v]_{\mathcal E}
					=PA[\vec v]_{\mathcal V}
					=P[A\vec v]_{\mathcal V}
					=[A\vec v]_{\mathcal E}.
				\]
		...
		\item Can you find a matrix $D$ so that
			...
			\]
			\begin{solution}
				$D=\mat{2&0&0\\0&-1&0\\0&0&1}$ is such a matrix. 

				We know, for example, that $A$ acts on $\vec v_1$ by multiplying
				it by $2$. $P^{-1}\vec v_1=[\vec v_1]_{\mathcal V}=\mat{1\\0\\0}$,
				so we should have that $D\mat{1\\0\\0}=\mat{2\\0\\0}=[2\vec v_1]_{\mathcal V}$. 
				Then $P$ transforms this back into $[2\vec v_1]_{\mathcal E}$. 

				By similar reasoning, the second and third columns of $D$ should be
				$\mat{0\\-1\\0}$ and $\mat{0\\0\\1}$.  
			\end{solution}
	...
	\begin{parts}
	\item Is $T$ diagonalizable (i.e., similar to a diagonal matrix)? 
		If so, explain how to obtain its diagonalized form.
		\begin{solution}
			Yes. 

			Let $\mathcal V=\Set{\vec v_1,\ldots \vec v_n}$ be the basis consisting
			of eigenvectors of $T$. Then $T$ acts on each of these by multiplying 
			them by the appropriate eigenvalue: $T\vec v_i=\lambda_i\vec v_i$. 

			So if $P$ is the transformation that takes vectors represented in the
			$\mathcal V$ basis and outputs their representations in the standard
			basis $\mathcal E$, we should have that, for example, 
			\[
				P^{-1}TP\mat{1\\0\\\vdots\\0}
				=P^{-1}TP[\vec v_1]_{\mathcal V}
				=P^{-1}T[\vec v_1]_{\mathcal E}
				=\lambda_1P^{-1}[\vec v_1]_{\mathcal E}
				=\lambda_1[\vec v_1]_{\mathcal V}
				=\mat{\lambda_1\\0\\\vdots\\0}.
			\]
			Therefore, the first column of $P^{-1}TP$ is $\mat{\lambda_1\\0\\\vdots\\0}$.
		...
		\item What if the eigenvectors of $T$ did not form a basis for $\R^n$.
			...
			\begin{solution}
				...
				Moreover, suppose $T$ were similar to diagonal matrix. That is, 
				suppose there were a diagonal matrix $D$ and an invertible matrix
				$P$ such that $T=PDP^{-1}$. Then, if $\vec v_1$ is the first column
				of $P$ and $\lambda_1$ is the first entry on the diagonal of $D$,
				we would have
				\[
					T\vec v_1
					=PDP^{-1}\vec v_1
					=PD\mat{1\\0\\\vdots\\0}
					=\lambda_1P\mat{1\\0\\\vdots\\0}
					=\lambda_1\vec v_1,
				\]
