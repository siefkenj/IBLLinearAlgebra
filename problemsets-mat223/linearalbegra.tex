	...
	\begin{parts}
		\item Find a matrix $P$ so that $P\vec x=\mathcal P(\vec x)$ for all $\vec x\in\R^2$.
			\begin{solution}
				...
				$\mathcal P(\vec x)=\dfrac{\vec x\cdot\vec u}{\norm{u}^2}\vec u$.
				So if $\Set{\vec e_1,\vec e_2}$ is the standard basis for $\R^2$,
				we can compute
				\[
					\mathcal P(\vec e_1)=\frac{2}{13}\vec u=\frac{1}{13}\mat{4\\6}
					\qquad \text{and} \qquad
					\mathcal P(\vec e_2)=\frac{3}{13}\vec u=\frac{1}{13}\mat{6\\9}.
				\]
				So since we know $\mathcal P$ is linear, for any $\vec x=\mat{x\\y}$,
				we can compute:
				\[
					\mathcal P(\vec x)
					=x\mathcal P(\vec e_1)+y\mathcal P(\vec e_2)
					=\frac{x}{13}\mat{4\\6}+\frac{y}{13}\mat{6\\9}.
				\]
				The matrix that acts like this is $P = \frac{1}{13}\mat{4&6\\6&9}$.
			\end{solution}
		\item Find a matrix $R$ so that $R\vec x=\mathcal R(\vec x)$ for all $\vec x\in\R^2$.
			\begin{solution}
				$R=\mat{0&-1\\1&0}$ is such a matrix, 
				found using the same reasoning as the previous part
			\end{solution}
		\item Write down matrices $A$ and $B$ for $\mathcal P\circ\mathcal R$ 
			...
			\begin{solution}
				...
				are two such matrices, found using the same reasoning as above.
			\end{solution}
		\item How do the matrices $A$ and $B$ relate to the matrices $P$ and $R$?
			Using the results of the two parts, we have
			\begin{solution}
				$A = PR$ and $B = RP$. 
