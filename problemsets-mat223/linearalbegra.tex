	...
	\begin{parts}
		...
		\item Describe $\Span\Set{\vec v_1,\vec v_3}$.
			\begin{solution}[inline]
				...
				a line through the origin along $\vec v_1$. 
			\end{solution}
...
\end{lesson}
	...
	\begin{parts}[resume]
		...
		\item Is $\Set{\vec u,\vec v,\vec w,\vec r}$ linearly independent?
			\begin{solution}
				No. We already know $\Set{\vec u,\vec v,\vec w}$ is not linearly
				independent, so no set that contains it can be linearly independent.
			\end{solution}
	\end{parts}
