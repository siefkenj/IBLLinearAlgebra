...
\end{lesson}
	...
	\begin{parts}
		\item Describe the row space of $A$.
			\begin{solution}
				The row space of $A$ is $\Span\Set*{\mat{1\\0\\0},\mat{0\\1\\0}}$,
				which is the $xy$-plane in $\R^3$. 
			\end{solution}
		\item Describe the column space of $A$.
			\begin{solution}
				The column space of $A$ is $\Span\Set*{\mat{1\\0},\mat{0\\1},\mat{0\\0}}
		...
		\item Is the row space of $A$ the same as the column space of $A$?
			\begin{solution}
				...
				Although they are both two dimensional spaces, the row space is a
				subspace of $\R^3$ and all vectors in it have three coordinates 
				(with the third always being zero), while the column space is a
		...
		\item Describe the null space of $A$.
			\begin{solution}
				...
				A vector $\vec x=\mat{x\\y\\z}$ is in the null space of $A$ if
				and only if
				\[
	...
	\begin{parts}
		\item How does the row space of $B$ relate to the row space of $C$?
			\begin{solution}
				...
				some row operations, the row space of $C$ is a subset of the row
				space of $B$. 

				Since row operations are all reversible, we also know that $B$ 
				can be obtained from $C$ by applying row operations, and so the
				row space of $B$ is also a subset of the row space of $B$.
		...
		\item How does the null space of $B$ relate to the null space of $C$?
			\begin{solution}
				...
				A vector is in the null space of $B$ or the null space of $C$ if
				and only if it is orthogonal to all vectors in the row space of
				$B$	or all vectors in the row space of $C$, respectively. 
				But the row spaces of $B$ and $C$ are equal by the previous part,
				and so their null spaces must also be equal. 
			\end{solution}
		\item Compute the null space of $B$.
			\begin{solution}
				We compute the null space of $C$, since it equals the null space
				of $B$ by the previous part. 

				$\vec x=\mat{x\\y\\z}$ is in the null space of $C$ if and only if
				\[
