	...
	\begin{parts}
		...
		\item Find $\Proj_\ell \vec v$. (Recall that a quadratic $at^2+bt+c$ has 
			...
			\begin{solution}
				...
				$t = 1$, so $1 \vec a = \vec a$ is the closest point in the line
				to $\vec v$. 
			\end{solution}
		\item Is $\vec v-\Proj_\ell \vec v$ a normal vector for $\ell$?
			...
			\begin{solution}
				...
				By the previous parts, $\vec v-\Proj_\ell \vec v = \mat{0\\1}$.
