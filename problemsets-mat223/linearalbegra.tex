	...
	\begin{parts}
		\item Is $F$ diagonalizable? Why or why not?
			\begin{solution}
				...
				$\chr(F)=(1-\lambda)^2$, so the only eigenvalue of $F$ is $1$, 
				meaning that the eigenvectors of $F$ are precisely the non-zero
				vectors in $\Null(F-1I)$. We check that 
				\[
					\Null(F-1I)
		...
		\item Suppose $A$ is a matrix where the geometric multiplicity of one of its eigenvalues
			...
			\begin{solution}
				...
				algebraic multiplicity, $A$ cannot be diagonalizable. Over the 
				complex numbers, the sum of the algebraic multiplicites of an 
				$n \times n$ matrix is always $n$. If one of the geometric multiplicities
				...
				matrix is diagonalizable. Since the sum of the algebraic multiplicites 
				of an $n \times n$ matrix over the complex numbers is always $n$,
				the sum of the geometric multiplicities would also equal $n$ in
				this case. So if we pick a basis for each eigenspace, their union
				would form a linearly independent set of size $n$ (i.e., a basis
