...
\IfSubStr{\jobname}{\detokenize{solutions}}{\setsolutions}{
	\IfSubStr{\jobname}{\detokenize{instructor}}{\setinstructor}{
		\IfSubStr{\jobname}{\detokenize{tophat}}{\settophat}{
 			\setstandard
 		}
	}
}
...
\iftoggle{tophat}{
	...
	\voidenvironment{annotation}
	\voidenvironment{lesson}
	\voidenvironment{iola}
	
	

	\let\oldquestion\question 
	%Redfine \question to add a \newpage to all questions, and take up a bit less space 
	\renewcommand{\question}{%
		\newpage %adds a newpage to the question command
		...
		\makebox(.5,0)[lt]{
			\hfill
			\hspace{-9mm} %less indent
			\makebox(0,10)[lt]{}%\rule{1.4in}{.5pt}} No line
	...
	\let\oldsection\section %Make it so sections and subsections don't appear
	\let\oldsubsection\subsection
	\renewcommand{\subsection}[2]{  }
	\renewcommand{\section}[2]{  }
	
	\AtBeginEnvironment{definition}{\newpage} %add a newpage to defns and thms
	\AtBeginEnvironment{theorem}{\newpage}
	
	
	

	\geometry{papersize={110mm,200mm},top=0mm,bottom=0mm,left=11mm,right=0mm,marginpar=0mm}
	
	\renewcommand{\coversubtitle}{}
	
	
	\usepackage{blowup} %Makes the page size and text 3x bigger, which seems to be about right for image resolution that works well on tophat (if the resolution is too small, it looks pixely and if the resolution is too large tophat only displays a piece of the question instead of the whole question
	\blowUp{target=x3}
}{}
%\voidenvironment{solution}
%\voidenvironment{annotation}
...
\section*{Lines and Planes}
	...
	\begin{parts}
		...
		\item For which $t\in\R$ is it true that $\vec a+t\vec d\in A$? Explain using your picture.
			\begin{solution}
				$\vec a +t\vec d\in A$ for any $t\in \R$. We can see this because if we start at the
...
\end{lesson}
	...
	\quad \overbrace{\vec x=t\mat{-1\\1\\1}+\mat{-1\\1\\2}}^{\displaystyle B}
	\quad \overbrace{\vec x=t\mat{2\\-1\\1}+\mat{1\\1\\1}}^{\displaystyle C}.
	\]
	\begin{parts}
...
\section*{Span}
	...
	\begin{definition}[Span]
		...
		\iftoggle{tophat}
		{In set notation: $\Span V$ is defined to be the set of vectors:
		\[
			\Set{\vec v \given \vec v=\alpha_1\vec v_1 + {\scriptstyle\cdots}
			+\alpha_n\vec v_n \, \text{\small{for some}} \, \vec v_1,{\scriptstyle\ldots},\vec v_n\in V
			\, \text{\small{and scalars}} \, \alpha_1,{\scriptstyle\ldots},\alpha_n}
		\]
		}
		{That is, \[
...
\vspace{-1.5em}
	...
	\begin{annotation}
		...
		\end{notes}
	\end{annotation}
	Classify the following as linear transformations or not.
	\begin{parts}
		\item 
			\begin{enumerate}
