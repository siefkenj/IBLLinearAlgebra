...
\begin{exercises}
	...
	\begin{problist}
		...
		\prob Give an example of a $3x4$ matrix $M$ with the specified rank, or explain why it cannot exist.
		\begin{enumerate}
			\item $\Rank(M) = 0$
			\item $\Rank(M) = 1$
			\item $\Rank(M) = 3$
		...
		\prob Consider the linear transformation $P:\R^3\to\R^3$, the projection onto the $xy$-plane.
		\begin{enumerate}
