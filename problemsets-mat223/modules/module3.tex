...
the equations is always \emph{consistent}.}, any vector in $\R^2$ can be obtained as a linear combination of $\vec u$ and $\vec v$.
...
\begin{example}
	...
	If $y=-2x$, then the vector is of the form $t\mat{1\\-2}$ for some $t$, which is just a multiple of $\vec v$. Thus
	\[
		\Span\Set{\vec u,\vec v}=
		\Set{\vec x\given \vec x=t\vec v\text{ for some }t\in\R}.
	\]
\end{example}
...
\begin{example}
	...
	from the coordinates we get the system
	\[
		\sysdelim..\systeme*{x=\alpha_1+\alpha_3, y=2\alpha_1+\alpha_2+\alpha_3, z=\alpha_1+2\alpha_3}
	\]
	solving for $\alpha_1, \alpha_2$, and $\alpha_3$, we find that the original equation is
	always consistent. Namely,
	\[
		\sysdelim..\systeme*{\alpha_1=2x-z, \alpha_2=-3x+y+z, \alpha_3=-x+z}
	\]
	Thus, $\R^3=\Span\Set{\vec a,\vec b,\vec c}$.
\end{example}
...
\begin{example}
	...
	The equation $x+2y=0$ can be written as $y=-\frac{1}{2}x$. Therefore
	\[
		\ell_1=\Set*{\vec x\given \vec x={t\mat{2\\-1}\text{ for some }t\in \R}}
		= \Span\Set*{\mat{2\\-1}}
	\]
\end{example}
...
\begin{example}
	...
	$4(0)-2(0) = 0 \neq 6$, therefore $\ell_2$ does not pass through the origin.
	Every span contains the origin, thus $\ell_2$ cannot be written as a span.
\end{example}
...
a corresponding point in $\ell_1$. Using the idea of set addition, we can express this mathematically with the equation
\[
