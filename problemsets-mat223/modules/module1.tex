...
\begin{example}
	...
	\]
	Substituting $\vec r$, $\vec x$, and $\vec y$ , we see this is equivalent to finding scalars $a$ and $b$ so that 
	\begin{align*}
	    2\xhat+2\yhat =a(\xhat-\yhat)+b(3\xhat-\yhat)=(a+3b)\xhat + (a+b)\yhat.  
	\end{align*}
	Solving the system
	\[
	    \sysdelim\{.
	...
	\]
	we have $a=-4$ and $b=2$ as the only solution. Therefore, $\vec r$ is a linear combination of $\vec x$ and $\vec y$. More specifically, 
	\[
