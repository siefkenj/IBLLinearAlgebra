...
\begin{example}
	...
	\]
	Substituting for $\vec x$ and $\vec y$, we see this is equivalent to finding scalars $a,b$ so that 
	\[
	    2\xhat+2\yhat=(a+3b)\xhat+(-a-b)\yhat.
	\]
	Rewriting the vector equation as a system of linear equations, we see we need to solve
	\[
	    \sysdelim\{.
		\systeme{
			a+3b=2,    
	...
	\]
	which has $a=-4$ and $b=2$ as the only solution. Therefore, $\vec r$ is a linear combination of $\vec x$ and $\vec y$. More specifically, 
	\[
