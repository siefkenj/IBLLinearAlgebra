...
\begin{example}
	...
	$\vec r = \alpha_{1}\vec x + \alpha_{2}\vec y$, then we have shown that $\vec r$ is a linear combination of $\vec x$ and $\vec y$.
	...
	2 &= -\alpha_{1}-\alpha_{2}
	\end{align*}
	Using your favourite method to solve a system of equation (i.e substitution, elimination, row reduction etc.) we find that $\alpha_{1}=-4$ and $\alpha_{2}=2$.
	We can check that our answer is correct by substituting our values back into the original equation and indeed we see that $\vec r = -4\vec x + 2\vec y$ thus, $\vec r$ is a linear combination of $\vec x$ and $\vec y$.
