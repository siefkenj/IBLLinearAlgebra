...
\begin{example}
	...
	Recall that $C_2$ is the unit square in $\R^2$ with sides given by $\vec \xhat = \mat{1\\0}$ and $\vec y_hat = \mat{0\\1}$. Applying the linear transformation $\mathcal T$ to $\vec xhat$ and $\vec yhat$, we obtain
	\[
	    \mathcal T(\vec xhat)=\mat{2\\-1} \qquad\text{and}\qquad \mathcal T(\vec yhat)=\mat{1\\\frac{1}{2}}.
	\]
