...
\begin{example}
	...
	As we discussed above, $C_2$ is the unit square in $\R^2$ with sides given by $\vec e_1 = \mat{1\\0}$ and $\vec e_2 = \mat{0\\1}$. Apply the linear transformation $\mathcal T$ to $\vec e_1$ and $\vec e_2$, we obtain
	\[
	    \mathcal T(\vec e_1)=\mat{2\\-1} \qquad \mathcal T(\vec e_2)=\mat{1\\\frac{1}{2}}.
	\]
	Plot $\mat{2\\-1}$ and $\mat{1\\\frac{1}{2}}$ as follows:
	
	XXX Figure
	
	We obtain that the volume of $\mathcal T(C_2)$ is 2.
\end{example}
...
\begin{example}
	...
	Similarly to the earlier example, we compute $\mathcal S\mat{1\\0} = \mat{-1\\1}$ and $\mathcal S\mat{0\\1} = \mat{1\\1}$. Then, Plot them in the graph and we may notice that the orientation of $\mathcal S$ is reversing. Thus, $\det(\mathcal S) = - \Vol(\mathcal S(C_2)) = -2$.
\end{example}
\begin{example}
...
\begin{example}
	...
	By applying row reduction to $A$, we could obtain
	\[
	    \mat{1&-2\\0&1}\mat{1&0\\0&-\frac{1}{2}}\mat{1&0\\-3&1}\mat{1&2\\3&4} = \mat{1&0\\0&1}.
	\]
	Therefore, by the theorem above, we would get
	\[
	    1\cdot(-\frac{1}{2})\cdot1\cdot\det(A) = 1,
	\]
	That is, $\det(A) = -2$.
\end{example}
