...
\begin{example}
	...
	Recall that $C_2$ is the unit square in $\R^2$ with sides given by $\vec e_1 = \mat{1\\0}$ and $\vec e_2 = \mat{0\\1}$. Applying the linear transformation $\mathcal T$ to $\vec e_1$ and $\vec e_2$, we obtain
	\[
	    \mathcal T(\vec e_1)=\mat{2\\-1} \qquad\text{and}\qquad \mathcal T(\vec e_2)=\mat{1\\\frac{1}{2}}.
	\]
