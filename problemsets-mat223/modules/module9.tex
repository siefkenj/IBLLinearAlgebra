...
\begin{example}
	...
	Let vectors $\vec u=\mat{u_1\\u_2},\vec v=\mat{v_1\\v_2}$ and scalar $\alpha$ be given. So $\vec u +\vec v=\mat{u_1+v_1\\u_2+v_2}$ and $a\vec u = \mat{a u_1\\a u_2}$. We then have 
	\[
	    \mathcal T(\vec u +\vec v)=\mat{2u_1+2v_1\\u_2+v_2}=\mat{2u_1\\u_2}+\mat{2v_1\\v_2}=\mathcal T(\vec u) + \mathcal T(\vec v)
	\]
	and
	\[
	    \mathcal T(a\vec u)=\mat{2a u_1\\a u_2}=a\mat{2 u_1\\ u_2}=a\mathcal T(\vec u).
	\]
	Therefore, $\mathcal T$ is linear.
	
	For $\mathcal S$:
	
	We could easily find two counter examples such as:
	\[
	    \mathcal S(\xhat +\yhat)=\mat{1\\5} \neq \mat{1\\4}+\mat{0\\5}=\mathcal S(\xhat) + \mathcal S(\yhat)
	\]
	or
	\[
	    \mathcal S(3\xhat)=\mat{3\\4} \neq 3\mat{1\\4}=a\mathcal S(\xhat).
	\]
	Either of them is enough to prove that $\mathcal S$ is not linear.
\end{example}
