\begin{exercises}
	...
	\begin{problist}
		...
		\end{solution}
	        ...
	        \prob Let $\vec u, \vec v \in \R^n$. Prove $|\vec u \cdot \vec v| \leq \|\vec u \|\|\vec v\|$, the \emph{Cauchy-Schwarz inequality}. Hint: use the geometric formula of the dot product of $\vec u, \vec v$.

	        \prob Let $\vec u, \vec v \in \R^n$. Prove $\|\vec u + \vec v\| \leq \|\vec u \| + \|\vec v\|$, the \emph{triangle inequality} in $\R^n$. Hint: use the \emph{Cauchy-Schwarz inequality}.

	        \prob Suppose $\mathcal {A} = \{\vec v_1,\dots , \vec v_k\} \subset \R^n$ is a set of nonzero, mutually orthogonal vectors, i.e., for all $i \neq j$, $\vec v_i \cdot \vec v_j = 0$. Prove $\mathcal{A}$ is a linearly independent set.
	\end{problist}
\end{exercises}
