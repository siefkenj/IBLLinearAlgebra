...
\begin{example}
    ...
    since $\cos \theta$ is a monotonically decreasing function for $\theta\in[0,\pi]$, we only need to compute the cosines of angles $\alpha$, $\beta$ and $\gamma$ between $\vec a$, $\vec b$, $\vec c$ and $\vec v$ respectively, and find the largest one.
    	
    First, by equating the geometric and algebraic definitions of the dot product, we know 
    \[
