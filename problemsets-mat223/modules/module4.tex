...
\begin{example}
	...
	To find the vector, we need to compute the angles $\alpha$, $\beta$ and $\gamma$ between vectors $\vec a$, $\vec b$, $\vec c$ and $\vec v$ respectively. 
	
	Similarly to the example above, equating the algebraic and geometric definitions of the dot product, we see
	\begin{align*}
	    &\cos \alpha = \frac{\vec a\cdot\vec v}{\norm{\vec a}\norm{\vec v}} = \frac{3+8}{5\sqrt{5}}=\frac{11\sqrt{5}}{25} \approx 0.9838699101 \\
	    &\cos \beta = \frac{\vec b\cdot\vec v}{\norm{\vec b}\norm{\vec v}} = \frac{9+12}{5\cdot 3\sqrt{2}} =\frac{7\sqrt{2}}{10} \approx 0.989949437\\
	    &\cos \gamma = \frac{\vec c\cdot\vec v}{\norm{\vec c}\norm{\vec v}} = \frac{6+4}{5\sqrt{5}}=\frac{2\sqrt{5}}{5} \approx 0.894427191. 
	\end{align*}
	Since $\cos x$ is a decreasing function for $x\in[0,\pi]$, we know that $\beta$ is the smallest angle because $\cos \beta$ gives the highest value. That is, $\vec b$ has a direction closest to the direction of $\vec v$.
\end{example}
