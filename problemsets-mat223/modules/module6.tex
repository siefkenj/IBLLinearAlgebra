...
\begin{example}
	...
	We are looking for a set of linearly independent vectors which spans $\ell$.
	Notice that $\mat{1\\2}$ is linearly dependent of $\mat{-2\\-4}$
	and linearly dependent of $\mat{1/2\\1}$.
	This tells us that if a basis for $\ell$ contains one of these three vectors,
	it cannot contain any of the other two.
    ...
    \[
		...
		\ell
    \]
    due to the linear dependence among $\mat{1\\2}$, $\mat{-2\\-4}$, and $\mat{1/2\\1}$. 
	And since $\Span\Set*{\mat{1\\2}} = \ell$,
	we can confirm that $\Set*{\mat{1\\2}}$ is a suitable basis for $\ell$.
	We can say the same for $\Set*{\mat{1/2\\1}}$,
	since $\Span\Set*{\mat{1/2\\1}} = \ell$.
    Let's call these bases $\mathcal B_1$ and $\mathcal B_2$, respectively.
    \[
...
\begin{example}
	...
	Find the dimension of subspace $\ell$.

	This is the same subspace from the earlier example where we found
	$\mathcal B_1 = \Set*{\mat{1\\2}}$ and $\mathcal B_2 = \Set*{\mat{1/2\\1}}$
	were suitable bases for $\ell$.
	$\mathcal B_1$ contains one element; therefore,
	$\ell$ is a one dimensional subspace\footnote{
	We could have considered $\mathcal B_2$ or one of the many other bases for $\ell$
