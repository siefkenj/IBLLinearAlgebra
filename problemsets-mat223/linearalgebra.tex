...
\end{lesson}
	...
	\[
		\ell_1 = \Span\Set{\vec d}\qquad\text{and}\qquad \ell_2=\Span\Set{\vec p}+\Set{\vec d}.
	\]
	\begin{parts}
	...
	\begin{annotation}
		...
		\begin{notes}
			\begin{itemize}
				\item Students have already worked witht this plane in problem \ref{ProbPlane},
...
\begin{lesson}
	...
	left-over dimenstions of the domain are not accounted for by the rank end up in the null space.
	This is valuabel to know because a matrix equation $A\vec x=\vec b$ has infinitely many
	solutions if and only if $A\vec x=\vec b$ is consisten and $A$ has a non-trivial null space. Using
	the rank-nullity theorem, we can connect the rank of a matrix to wheither a matrix equation has infinitely
	...
	\begin{parts}
		\item Find the oriented volumes of $M(C_2)$, $T(C_2)$, and $S(C_2)$.
			\begin{solution}
				...
				We can compute the the volume of $M(C_2)$ directly, since $M$
