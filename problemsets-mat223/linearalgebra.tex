...
\section*{Lines and Planes}
	...
	\begin{parts}
		...
		\item Add the vectors $\vec a=\mat{-1\\-1}$, $\vec b=\mat{1\\3}$ and
			...
			\begin{solution}
			    ...
			    \begin{axis}[scale=1,
				    ...
				    \draw[Green, very thick] (-5,-9) -- (5,11) node[pos=.3, above left] {$A$};
		...
		\item For which $t\in\R$ is it true that $\vec a+t\vec d\in A$? Explain using your picture.
			\begin{solution}
				$\vec a +t\vec d\in A$ for all $t\in \R$. We can see this because if we start at the
				vector $\vec a$ and the displace by $t\vec d$, we will always be on the line $A$.
			\end{solution}
	\end{parts}
		...
		\begin{enumerate}
			\item Draw a picture of $\vec a $ and $\vec b$.
			    ...
			    \begin{axis}[scale=.5,
				    ...
				    \draw[Blue, very thick, ->] (0,0) -- (3,2) node[near end, below] {$\vec b$};
			    \end{axis}
			\end{tikzpicture}
				\end{solution}
...
\end{lesson}
	...
	\begin{annotation}
		\begin{goals}
			...
			The goal of this problem is to
			\begin{itemize}
				\item Determine the orientation of a basis froma  pciture.
...
\begin{lesson}
	...
	\Heading{Motivation}
		...
		the orthogonal compliment to the null space (which will allow us to tie together normal form and vector
		...
		\begin{notes}
			\begin{itemize}
				\item In part 2, encourage students to think both geometrically
					...
					$C\vec x=\vec 0$ relate? What is the orthogonal compliment to their row
					spaces?
			\end{itemize}
		\end{notes}
	\end{annotation}
...
\end{lesson}
	...
	\begin{annotation}
		...
		\begin{notes}
			\begin{itemize}
				\item Don't labor over proofs in this quesiton.
			\end{itemize}
		\end{notes}
	\end{annotation}
	...
	\begin{annotation}
		...
		\begin{notes}
			\begin{itemize}
				\item Part (d) will be the only hard one; don't spend much time on this quesiton.
			\end{itemize}
		\end{notes}
	\end{annotation}
	\begin{parts}
...
\begin{lesson}
	...
	\Heading{Motivation}
	A common mathematical technique is to decompose a complicated problem into lost of simpler ones.
...
\end{lesson}
	\question
	Let $\mathcal S=\Set{\vec e_1,\vec e_2,\ldots,\vec e_n}$ be the standard basis for $\R^n$.
	...
	\begin{parts}
		\item Should $X^{-1}$ exist? Explain.
			\begin{solution}
				Yes. $X$ converts vectors from the standard basis into the
				$\mathcal B$ basis, and this process can be undone.
		...
		\item Consider the equation
			...
			\begin{solution}
				$X^{-1}[\vec v]_{\mathcal S} = [\vec v]_{\mathcal B}$.
...
\end{center}
	...
	\begin{parts}
		...
		\item What is the volume of the image of the unit square
			(i.e., the volume of $T(C_2)$)? You may need to use trigonometry.
			\begin{solution}
	...
	\begin{parts}
		...
		\item Compute $\det(A)$.
			\begin{solution}
				...
				The paralellogram with sides $\mat{2\\0}$ and $\mat{0\\1}$ is
...
\end{lesson}
	...
	following about the linear transformations $M$, $T$, and $S$: $M$ is defined
	...
	\begin{parts}
		...
		\item How does the volume of $T(C_2+\{\vec e_1\})$ compare to the volume
			of $T(C_2)$?
			\begin{solution}
				They are equal. The transformation $T$ scales the volumes of all
				regions by $2$.
			\end{solution}
	...
	\begin{parts}
		...
		\item Suppose $Y$ is not invertible. What is $\det(Y)$?
			\begin{solution}
				...
				This makes sense because for a square matrix $Y$,
				$\det(Y^{-1})=\frac{1}{\det(Y)}$ by the previous part. This
...
\end{lesson}
	...
	\begin{parts}
		...
		\item Find the characteristic polynomial of $A$.
			\begin{solution}
				$\chr(A)=-(\lambda-2)(\lambda+1)(\lambda-1)$.

				We compute
				\[
					\chr(A)
					=\det(A-\lambda I)
					=\det\left(\mat{1-\lambda&0&1\\0&1-\lambda&1\\1&1&-\lambda}\right)
		...
		\item Compute $A\vec u$ where $\vec u=a\vec v_1+b\vec v_2+c\vec v_3$ for
			...
			\begin{solution}
				$A\vec u=2a\vec v_1-b\vec v_2+c\vec v_3$.
...
\begin{lesson}
	...
	\Heading{Motivation}
	We finally have all the tools we need to diagonalize a matrix, and explain why diagonalization is useful.
	Let's do it!
...
\begin{lesson}
	...
	\begin{annotation}
		...
		\end{notes}
	\end{annotation}
	Diagonalization is great, but not all matrices can be diagonalized. This is because the we might not
	have a basis of eigenvectors. The sum of the geometric multiplicities gives the dimension of the span
	of all the eigenvectors. If it is not $n$ (for an $n\times n$ matrix), the we cannot be diagonalizable.
