...
\end{lesson}
	\begin{definition}[Vector Form of a Line]
		Let $\ell$ be a line and let $\vec d$ and $\vec p$ be vectors.
		If $\ell=\Set{\vec x\given \vec x=
		t\vec d+\vec p\text{ for some } t\in\R }$ we say the equation
		\[
			\vec x=t\vec d+\vec p
		\]
		is $\ell$ expressed in \emph{vector form}.
		The vector
...
\end{lesson}
	...
	\begin{parts}
		...
		\item $[\vec x]_{\mathcal V}=\mat{1\\3\\4}$.  Compute $T_A^{100}\vec x$. Express
			your answer in both the $\mathcal V$ bais and the standard basis.
			\begin{solution}
	...
	\begin{parts}
		...
		\begin{solution}
			...
			are also the same, execpt eigenvectors for $B$ must be written
...
\end{lesson}
	...
	Let $A$ be an $n\times n$ matrix with eigenvalues $\lambda_1,\ldots,\lambda_m$.  The
	...
	of the corresponding eigenspace.  The \emph{algebraic multiplicity}
