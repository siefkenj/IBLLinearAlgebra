...
\begin{document}
	...
	\setheader{MAT223}{Writing Assignment 2}{Due: Wed.~October 23\textsuperscript{rd}, 2019}{}{Name:\underline{\hspace{2in}}}{}
	...
	\item $\vec{v}$ points vertically.
	\item A plane has two directions.
	\item $\vec{x} + \mat{5 \\ 0}$ is a translation.
	\end{enumerate}
...
\item[\bf (PAR) Part 2] \emph{\color{blue} Prepare your response to this part on the PAR Worksheet. You will bring your writeup of this
	...
	Jordan, Max, and Lily are having a conversation at the breakfast table while admiring a new tablecloth. 

	\begin{center}
    \begin{tikzpicture}[xscale=1.5,yscale=.75]
    	\draw[step=1cm,gray,very thin] (-3,-3) grid (3,3);
	    \node at (2,-1) {\includegraphics[width=1cm]{mug.png}};
	    \node at (1,2) {\includegraphics[width=1cm]{plate.png}};
		\node at (0,0)[circle,fill,minimum size=6pt, inner sep=0pt, outer sep=0pt]{};
		\draw[very thick] (-3,0) -- (3,0) (0,3) -- (0,-3);
		\node at (0,-3.5) {Lily};
		\node[rotate=-90] at (-3.25,0) {Max};
		\node[rotate=180] at (0,3.5) {Jordan};
    \end{tikzpicture}
	\end{center}  

	\textsc{max:} ``Pass me the mug at $(1, 2)$, please.''

	\textsc{lily:} ``That's a plate.''

	\textsc{max:} ``No, the plate's at $(-2, 1)$.  The mug's right next to you.''
